...
def train_mle_estimator(train_images, train_labels):
    ...
    # train_images is (60000, 782), train_labels is (60000, 10)
    n = train_images.shape[0]
    pi_mle = np.sum(train_labels, axis=0) / n
    # we know that train_images.T @ train_labels = numerator of theta_mle
    theta_mle = train_images.T @ train_labels / (n * pi_mle.reshape(1, -1))
...
def train_map_estimator(train_images, train_labels):
    ...
    # we know that train_images.T @ train_labels = numerator of theta_mle
    theta_map = (2 + train_images.T @ train_labels) / (n * pi_map.reshape(1, -1) + 4)
...
def log_likelihood(images, theta, pi):
    ...
    # YOU NEED TO WRITE THIS PART
    n = images.shape[0]
    log_like = np.zeros((n, 10)) + np.log(pi.reshape((1, -1)))
    tmp = images @ np.log(theta) + (1 - images) @ np.log(1 - theta)
    log_like += tmp
    log_like -= np.log(np.exp(tmp) @ pi).reshape((-1, 1))
    return log_like
